...
Write to `references/<owner>-<repo>.md` (inside this skill's folder) as plain,
self-contained markdown: everything a reader needs is in the file itself, pasteable
into a separate conversation with no tool access. Quote real paths and lines, never
"see the repo".
...
3. Write a short, focused note answering the question, with file paths and quoted
   snippets, to `references/<owner>-<repo>.md`.
4. Delete the clone.
...
the findings to `references/<owner>-<repo>.md`, and deletes the clone. It collects:
...
**You write the Verdict section** from those findings, guided by
`references/checklist.md`. Follow up suspicious matches by reading the actual lines
in a fresh clone before judging - a keyword hit is a lead, not a conclusion.
