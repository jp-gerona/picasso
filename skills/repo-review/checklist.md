...
| Transitive dependencies | Runtime deps named in the manifest, one level down | The scan only sees this repo; a clean scan of a one-file library says nothing about its runtime deps (slugify review: verdict had to manually flag `@sindresorhus/transliterate`). Name them in the verdict and review separately before high-trust use. |
